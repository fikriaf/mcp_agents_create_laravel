"""
Utility functions for cleaning GenLaravel project
Moved from agents/clean_history.py
"""
...
def clean_laravel_generated_files():
    """
    Clean generated files from Laravel project
    Preserves welcome.blade.php
    """
    laravel_root = "my-laravel"

    # Clean components
    components_path = os.path.join(laravel_root, "resources/views/components")
    if os.path.exists(components_path):
        shutil.rmtree(components_path)
        print("🗑️ Cleaned: components/")

    # Clean layouts
    layouts_path = os.path.join(laravel_root, "resources/views/layouts")
    if os.path.exists(layouts_path):
        shutil.rmtree(layouts_path)
        print("🗑️ Cleaned: layouts/")

    # Clean generated blade files (preserve welcome.blade.php)
    views_path = os.path.join(laravel_root, "resources/views")
...
def reset_routes():
    """Reset routes to default with GenLaravel welcome"""
    laravel_root = "my-laravel"
...
"""
    ...
    with open(webphp_path, "w", encoding="utf-8") as f:
        f.write(default_routes)
    
    print("🗑️ Reset: routes/web.php")


def create_genlaravel_welcome():
    """Create GenLaravel branded welcome page"""
    laravel_root = "my-laravel"
    welcome_path = os.path.join(laravel_root, "resources/views/welcome.blade.php")
...
if __name__ == "__main__":
    print("🧹 Cleaning Laravel generated files...")
    clean_laravel_generated_files()
